...
class BuiltinEntity {
    ...
    init(name: String, flags: Entity.Flag = .none, constant: Value? = nil, type: Type, gen: @escaping (inout IRGenerator) -> IRValue) {
        let ident = Ident(start: noPos, name: name)
        let entity = Entity(ident: ident, type: type, flags: flags.union(.builtin), constant: constant)
        self.entity = entity
        self.type = type
        self.gen = gen
    }
