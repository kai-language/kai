...
extension FunctionPassManager {
    func addPasses(for level: OptimisationLevel) {
        ...
        add(.promoteMemoryToRegister, .basicAliasAnalysis, .instructionCombining, .reassociate)
        
        guard level >= .O2 else { return }
        
        add(.gvn, .cfgSimplification)
        
        guard level >= .O3 else { return }
        
        add(.loopUnroll, .tailCallElimination, .argumentPromotion)
    }
}
        ...
        if names.count > 1, values.count == 1, let value = values.first {
            ...
            for (entity, value) in zip(decl.entities, decl.initExprs) {
                ...
                if case .directive = value {
                    ...
                    let stringHeaderSize = 16 // 8 bytes for length 8 for capacity
                    ...
                    let arrayLengthIntPtr = builder.buildBitCast(tempArrayIr, type: PointerType(pointee: IntType.int64))
                    builder.buildStore(IntType.int64.constant(stringLength), to: arrayLengthIntPtr)
                    
                    let arrayCapacityIndex = builder.buildGEP(tempArrayIr, indices: [0, 8])
                    let arrayCapacityIntPtr = builder.buildBitCast(arrayCapacityIndex, type: PointerType(pointee: IntType.int64))
                    builder.buildStore(IntType.int64.constant(stringLength), to: arrayCapacityIntPtr)
                    
                    let arrayContentPointer = builder.buildGEP(tempArrayIr, indices: [0, stringHeaderSize])
                    
                    let tempArrayVoidIr = builder.buildBitCast(arrayContentPointer, type: PointerType.toVoid)
