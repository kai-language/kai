...
struct Lexer {
  ...
  mutating func next() throws -> Token? {
    ...
    case _ where identChars.contains(char):
      ...
      else if try symbol == Token.Keyword.infix.rawValue && peek() == .keyword(.operator) { return .keyword(.infix) }
      else if try symbol == Token.Keyword.prefix.rawValue && peek() == .keyword(.operator) { return .keyword(.prefix) }
      else if try symbol == Token.Keyword.postfix.rawValue && peek() == .keyword(.operator) { return .keyword(.postfix) }
